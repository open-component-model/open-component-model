...
// built by httpclient.New. Each top-level subtest covers one configuration
		...
		t.Run("tcpKeepAlive", func(t *testing.T) {
			for _, field := range []string{"tcpKeepAlive: 1s", "tcpKeepAlive: -1s"} {
				t.Run(field, func(t *testing.T) {
					...
					require.NoError(t, do(ctx, httpclient.New(httpclient.WithConfig(cfg)), registryURL))
				})
			}
		})
	})
...
// retry-enabled client httpclient.New builds from them.
func configuredClient(t *testing.T, fields ...string) *http.Client {
	t.Helper()
	return httpclient.New(httpclient.WithConfig(resolveConfig(t, fields...)))
}
...
func do(ctx context.Context, client *http.Client, url string) error {
...
func reusesConnection(t *testing.T, ctx context.Context, client *http.Client, url string, gap time.Duration) bool {
	t.Helper()
