...
`,
	...
	t.Run("retry fields", func(t *testing.T) {
		yamlCfg := `
type: generic.config.ocm.software/v1
configurations:
  - type: http.config.ocm.software/v1alpha1
...
`
		...
		assert.Equal(t, httpspec.NewTimeout(500*time.Millisecond), cfg.RetryMinWait)
		assert.Equal(t, httpspec.NewTimeout(10*time.Second), cfg.RetryMaxWait)
		assert.Equal(t, intPtr(3), cfg.RetryMaxRetry)
	})
...
`
		...
		assert.Equal(t, &httpspec.DefaultRetryMinWait, cfg.RetryMinWait, "RetryMinWait")
		assert.Equal(t, &httpspec.DefaultRetryMaxWait, cfg.RetryMaxWait, "RetryMaxWait")
		assert.Equal(t, &httpspec.DefaultRetryMaxRetry, cfg.RetryMaxRetry, "RetryMaxRetry")
	})
...
`
		...
		assert.Equal(t, httpspec.NewTimeout(100*time.Millisecond), cfg.RetryMinWait, "RetryMinWait")
		assert.Equal(t, httpspec.NewTimeout(5*time.Second), cfg.RetryMaxWait, "RetryMaxWait")
		assert.Equal(t, intPtr(10), cfg.RetryMaxRetry, "RetryMaxRetry")
	})
...
`
		...
		assert.Equal(t, httpspec.NewTimeout(50*time.Millisecond), cfg.RetryMinWait, "RetryMinWait")
		assert.Equal(t, httpspec.NewTimeout(1*time.Second), cfg.RetryMaxWait, "RetryMaxWait")
		assert.Equal(t, intPtr(3), cfg.RetryMaxRetry, "RetryMaxRetry")
	})
...
func TestDuration_MarshalRoundTrip(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("%v", time.Duration(*tt.duration)), func(t *testing.T) {
