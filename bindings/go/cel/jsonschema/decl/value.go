...
func (dv *DynValue) DeclType() *Type {
	return dv.declType
}
...
func exprValue(value interface{}) (ref.Val, *Type, error) {
	switch v := value.(type) {
	case bool:
...
var unknownType = &Type{name: "unknown", MinSerializedSize: 1}
...
func (sv *structValue) AddField(field *DynField) {
...
func (sv *structValue) GetField(name string) (*DynField, bool) {
...
func NewObjectValue(sType *Type) *ObjectValue {
	return &ObjectValue{
...
func (m *MapValue) ConvertToObject(declType *Type) *ObjectValue {
	return &ObjectValue{
...
// NewDynField returns a MapField instance with an empty DynValue that refers to the
// specified parse node id and field name.
func NewDynField(id int64, name string) *DynField {
	return &DynField{
		ID:   id,
		Name: name,
...
// DynField specifies a field name and a reference to a dynamic value.
