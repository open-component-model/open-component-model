...
const (
	...
	//nolint:gosec // G101: credential property key name, not a hardcoded credential.
...
func RootCertPoolFromCredentials(credentials map[string]string) (*x509.CertPool, error) {
	data, source, err := loadPEMBytes(credentials)
	if err != nil {
		return nil, err
	}
	if len(data) == 0 {
...
func loadPEMBytes(credentials map[string]string) ([]byte, string, error) {
	if val := credentials[CredentialKeyRootCertsPEM]; val != "" {
		return []byte(val), CredentialKeyRootCertsPEM, nil
	}
	if path := credentials[CredentialKeyRootCertsPEMFile]; path != "" {
		...
		return data, CredentialKeyRootCertsPEMFile, nil
	}
	return nil, "", nil
}
