...
func Test_CTF_Basic_ReadOnly_Compatibility(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			ctx := t.Context()
			r := require.New(t)
			archive, discovered, err := ctf.OpenCTFByFileExtension(ctx, tc.path, ctf.O_RDONLY, nil)
		...
		t.Run("work within "+tc.name, func(t *testing.T) {
			ctx := t.Context()
			r := require.New(t)
			err := ctf.WorkWithinCTF(ctx, tc.path, ctf.O_RDONLY, nil, func(ctx context.Context, ctf ctf.CTF) error {
...
func Test_CTF_Advanced_ReadOnly_Compatibility(t *testing.T) {
	t.Run("remote resource", func(t *testing.T) {
		ctx := t.Context()
		r := require.New(t)
		archive, err := ctf.OpenCTF(ctx, "testdata/compatibility/02/without-resource", ctf.FormatDirectory, ctf.O_RDONLY, nil)
		r.NoError(err)
		blobs, err := archive.ListBlobs(ctx)
		r.NoError(err)
		r.Len(blobs, 3)
	...
	t.Run("local (embedded) resource", func(t *testing.T) {
		ctx := t.Context()
		r := require.New(t)
		archive, err := ctf.OpenCTF(ctx, "testdata/compatibility/02/with-resource", ctf.FormatDirectory, ctf.O_RDONLY, nil)
		r.NoError(err)
		blobs, err := archive.ListBlobs(ctx)
		r.NoError(err)
		r.Len(blobs, 4)
