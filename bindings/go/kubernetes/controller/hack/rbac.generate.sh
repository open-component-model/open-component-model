#!/usr/bin/env bash
#
# post-process controller-gen RBAC output into the Helm
# chart's manager-role.yaml. Reads <src-dir>/role.yaml, rewrites metadata.name
# to the chart's templated resource name, and writes to
# <chart-dir>/templates/rbac/manager-role.yaml. Preserves controller-gen's
# native 2-space YAML indent.
#
...
"${YQ}" eval ".metadata.name = \"${NAME_SENTINEL}\"" "${SRC}" \
    | sed "s|${NAME_SENTINEL}|${NAME_TPL}|" > "${OUT}"
