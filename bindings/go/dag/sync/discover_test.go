...
import (
	...
	syncdag "ocm.software/open-component-model/bindings/go/dag"
)
...
func TestDAGDiscovery(t *testing.T) {
	...
	t.Run("graph discovery succeeds with provided graph", func(t *testing.T) {
		ctx := t.Context()
		r := require.New(t)
		// Emulate external dependency graph.
		// In real-world scenarios, this information would likely be retrieved from
		...
		externalGraph := NewSyncedDirectedAcyclicGraph[string]()
		dag := NewGraphDiscoverer(&GraphDiscovererOptions[string, string]{
			Roots: []string{"A", "B", "C", "D"},
			Graph: externalGraph,
			Resolver: ResolverFunc[string, string](func(ctx context.Context, key string) (value string, err error) {
		...
		var roots []string
		var vertices []string
		_ = externalGraph.WithReadLock(func(d *syncdag.DirectedAcyclicGraph[string]) error {
			roots = d.Roots()
			vertices = slices.Collect(maps.Keys(d.Vertices))
			return nil
		})
