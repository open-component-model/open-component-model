...
func (b *Builder) BuildAndCheck(original *v1alpha1.TransformationGraphDefinition) (*Graph, error) {
	...
	return &Graph{
		...
		credentialResolver: b.credentialResolver,
		events:             b.events,
	}, nil
}
...
// WithCredentialResolver sets the credential resolver used by the runtime to
// resolve credentials for TransformerWithCredentials implementations.
func (b *Builder) WithCredentialResolver(r credentials.Resolver) *Builder {
	b.credentialResolver = r
	return b
}
...
func (g *Graph) Process(ctx context.Context) error {
	...
	runtimeEvaluationProcessor := syncdag.NewGraphProcessor(synced, &syncdag.GraphProcessorOptions[string, graph.Transformation]{
		Processor: &graphRuntime.Runtime{
			...
			CredentialResolver:       g.credentialResolver,
