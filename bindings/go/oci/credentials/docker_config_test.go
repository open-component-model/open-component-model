...
func TestCredentialFromOCICredentials(t *testing.T) {
	tests := []struct {
		name     string
		creds    *credentialsv1.OCICredentials
		expected auth.Credential
	}{
		{
			name: "all fields",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := CredentialFromOCICredentials(tt.creds)
			assert.Equal(t, tt.expected, result)
		})
	}
}

func TestCredentialFromOCICredentials_MatchesFromDirectCredentials(t *testing.T) {
	...
	fromMap := CredentialFromMap(props)
	fromTyped := CredentialFromOCICredentials(credentialsv1.FromDirectCredentials(props))

	assert.Equal(t, fromMap, fromTyped, "typed and map paths should produce identical auth.Credential")
}
