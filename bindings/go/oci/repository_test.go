...
// TestRepository_AddLocalSource_OwnershipPolicyAddIfSupported_OCILayoutBody pins
// the regression that enabling OwnershipReferrerPolicyAddIfSupported must not
...
// hardcodes the policy to OwnershipReferrerPolicyNever so OwnershipReferrer
// is never invoked for sources, regardless of the repository's configured policy.
func TestRepository_AddLocalSource_OwnershipPolicyAddIfSupported_OCILayoutBody(t *testing.T) {
	r := require.New(t)
	ctx := t.Context()
	...
	repo := Repository(t,
		...
		oci.WithOwnershipReferrerPolicy(oci.OwnershipReferrerPolicyAddIfSupported),
	)
