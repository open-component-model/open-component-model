...
// A local blob is served from the local reference in its access, which is excluded
// from the normalised form a signature covers, while the resource digest is not. A
// redirected local reference must therefore not pass, even though the blob it points
// at verifies perfectly well against its own descriptor.
func TestRepository_GetLocalResource_RejectsRedirectedLocalReference(t *testing.T) {
	r := require.New(t)
	ctx := t.Context()
	...
	desc := &descriptor.Descriptor{
		...
		Component: descriptor.Component{
			Provider: descriptor.Provider{Name: "test-provider"},
			ComponentMeta: descriptor.ComponentMeta{
				ObjectMeta: descriptor.ObjectMeta{Name: "ocm.software/test-component", Version: "1.0.0"},
			},
		},
	}

	add := func(name string, payload []byte) *descriptor.Resource {
		res := &descriptor.Resource{
			Relation: descriptor.LocalRelation,
			ElementMeta: descriptor.ElementMeta{
				ObjectMeta: descriptor.ObjectMeta{Name: name, Version: "1.0.0"},
			},
			Type: "blob",
			Access: &v2.LocalBlob{
				LocalReference: digest.FromBytes(payload).String(),
				MediaType:      "application/octet-stream",
			},
		}
		stored, err := repo.AddLocalResource(ctx, desc.Component.Name, desc.Component.Version, res, inmemory.New(bytes.NewReader(payload)))
		r.NoError(err)
		return stored
	}

	honest := add("honest", []byte("the content the descriptor was signed over"))
	other := add("other", []byte("content from somewhere else entirely"))

	// The access is rewritten to serve the other blob while the digest, which a
	// signature would cover, still describes the original.
	redirected := honest.DeepCopy()
	redirected.Name = "redirected"
	redirected.Access = other.Access

	desc.Component.Resources = []descriptor.Resource{*honest, *redirected}
	r.NoError(repo.AddComponentVersion(ctx, desc))

	_, _, err = repo.GetLocalResource(ctx, desc.Component.Name, desc.Component.Version, honest.ToIdentity())
	r.NoError(err, "the untouched resource must still be served")

	_, _, err = repo.GetLocalResource(ctx, desc.Component.Name, desc.Component.Version, redirected.ToIdentity())
	r.ErrorContains(err, "digest mismatch")
}

func TestRepository_DownloadResource_DigestVerification(t *testing.T) {
	const artifactMediaType = "application/custom"
	payload := []byte("test content")

	// uploaded builds a repository holding one OCI image resource and returns the
	// resource as stored, whose digest is the digest of the manifest.
	uploaded := func(t *testing.T) (*oci.Repository, *descriptor.Resource) {
		t.Helper()
		r := require.New(t)
		ctx := t.Context()
		...
		buf := bytes.NewBuffer(nil)
		store, err := tar.NewOCILayoutWriterWithTempFile(buf, t.TempDir())
		r.NoError(err)

		base := content.NewDescriptorFromBytes("", payload)
		r.NoError(store.Push(ctx, base, bytes.NewReader(payload)))
		manifestDesc, err := oras.PackManifest(ctx, store, oras.PackManifestVersion1_1, artifactMediaType, oras.PackManifestOptions{
			Layers: []ociImageSpecV1.Descriptor{base},
		})
		r.NoError(err)
		r.NoError(store.Tag(ctx, manifestDesc, "test-image:latest"))
		r.NoError(store.Close())
		...
		res := &descriptor.Resource{
			...
			Type:   "ociImage",
			Access: &v1.OCIImage{ImageReference: "test-image:latest"},
		}

		stored, err := repo.UploadResource(ctx, res, inmemory.New(buf))
		r.NoError(err)
		r.NotNil(stored.Digest, "upload must record the manifest digest on the resource")
	...
	t.Run("rejects a manifest that does not match the resource digest", func(t *testing.T) {
		repo, res := uploaded(t)
		res.Digest.Value = digest.FromString("a manifest this registry never served").Encoded()

		// The check runs before any content streams, so the download itself fails.
		_, err := repo.DownloadResource(t.Context(), res)
		require.ErrorContains(t, err, "digest mismatch")
	})

	t.Run("serves a manifest unverified when the resource carries no digest", func(t *testing.T) {
		repo, res := uploaded(t)
		res.Digest = nil
	...
	t.Run("accepts the ociArtifactDigest spelling of the same digest", func(t *testing.T) {
		...
		// Descriptors in the wild carry the manifest digest under either
		// normalisation label, and neither may be rewritten.
