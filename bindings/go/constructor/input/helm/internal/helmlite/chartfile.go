...
func LoadChartfile(filename string) (*Metadata, error) {
...
func IsChartDir(dirName string) (bool, error) {
	...
	if _, err := os.Stat(chartYaml); os.IsNotExist(err) {
		return false, fmt.Errorf("no %s exists in directory %q: %w", ChartfileName, dirName, err)
	}

	chartYamlContent, err := os.ReadFile(chartYaml)
	if err != nil {
		return false, fmt.Errorf("cannot read %s in directory %q: %w", ChartfileName, dirName, err)
	}
