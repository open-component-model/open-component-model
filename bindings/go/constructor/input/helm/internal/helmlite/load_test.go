...
func verifyChart(t *testing.T, c *Chart) {
	t.Helper()
	if c.Name() == "" {
...
func verifyDependencies(t *testing.T, c *Chart) {
...
func verifyDependenciesLock(t *testing.T, c *Chart) {
	if len(c.Metadata.Dependencies) != 2 {
		t.Errorf("Expected 2 dependencies, got %d", len(c.Metadata.Dependencies))
	}
	tests := []*Dependency{
...
func verifyFrobnitz(t *testing.T, c *Chart) {
	verifyChartFileAndTemplate(t, c, "frobnitz")
}

func verifyChartFileAndTemplate(t *testing.T, c *Chart, name string) {
	if c.Metadata == nil {
...
func verifyBomStripped(t *testing.T, files []*File) {
