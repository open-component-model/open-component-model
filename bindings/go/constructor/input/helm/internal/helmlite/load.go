...
//
// This is the preferred way to load a  It will discover the chart encoding
...
func LoadFiles(files []*BufferedFile) (*Chart, error) {
	c := new(Chart)
	...
	for _, f := range files {
		...
		if f.Name == "Chart.yaml" {
			...
			// Because of that, if APIVersion is unset, we should assume we're loading a v1
	...
	for _, f := range files {
		...
		case f.Name == "requirements.yaml":
			...
			if c.Metadata.APIVersion != APIVersionV1 {
	...
	for n, files := range subcharts {
		...
		default:
			...
			// that is in charts/, but isn't actually a
