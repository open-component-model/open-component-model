...
func SaveDir(c *Chart, dest string) error {
	...
	for _, o := range [][]*File{c.Templates, c.Files} {
		for _, f := range o {
...
func writeTarContents(out *tar.Writer, c *Chart, prefix string) error {
	err := validateName(c.Name())
	if err != nil {
		return err
	}
	...
	if c.Metadata.APIVersion == APIVersionV2 {
		if c.Lock != nil {
