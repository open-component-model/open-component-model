...
func (i *Method) process(ctx context.Context, resource *spec.Resource, opts input.Options) (processed *descriptor.Resource, err error) {
	spec := v1.Helm{}
	if err := i.Scheme.Convert(resource.Input, &spec); err != nil {
		return nil, fmt.Errorf("error converting resource input spec: %w", err)
	}

	var helmChart *helmlite.Chart
	if spec.Path != "" {
		stat, err := os.Stat(spec.Path)
		if err != nil {
			return nil, fmt.Errorf("error checking helm chart source path %q: %w", spec.Path, err)
		}
		if stat.IsDir() {
			helmChart, err = helmlite.LoadDir(spec.Path)
		} else {
			helmChart, err = helmlite.LoadFile(spec.Path)
		}
		if err != nil {
			return nil, fmt.Errorf("error loading helm chart from path %q: %w", spec.Path, err)
		}
	}

	if helmChart == nil {
		return nil, fmt.Errorf("no valid helm chart found at path %q", spec.Path)
	}
	...
	metaDesc := content.NewDescriptorFromBytes(helmlite.ConfigMediaType, metaRaw)
	...
	if err := layout.Push(ctx, metaDesc, bytes.NewReader(metaRaw)); err != nil {
		return nil, fmt.Errorf("error writing helm config (%q) to layout: %w", helmlite.ConfigMediaType, err)
	}
	...
	archive, err := helmlite.Save(helmChart, tmp)
	if err != nil {
	...
	chartDesc := ociImageSpecV1.Descriptor{
		MediaType: helmlite.ChartLayerMediaType,
	...
	if err := layout.Push(ctx, chartDesc, chartData); err != nil {
		return nil, fmt.Errorf("error writing helm chart (%q) to layout: %w", helmlite.ChartLayerMediaType, err)
	}
	...
	if err := layout.Close(); err != nil {
		return nil, fmt.Errorf("error closing helm chart layout: %w", err)
	}

	return input.AddColocatedLocalBlob(ctx, opts.Target, opts.Component, opts.Version, resource, b)
}
