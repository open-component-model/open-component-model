...
func TestContextWithMultipleConfigurations(t *testing.T) {
	...
	r.Equal(new("/tmp/multi"), retrievedFsConfig.TempFolder, "filesystem config should be correct")
}
...
func TestContextOverwriteFilesystemConfig(t *testing.T) {
	...
	ocmCtx := FromContext(ctx)
	fsCfg := ocmCtx.FilesystemConfig()
	r.Equal(new("/tmp/initial"), fsCfg.TempFolder, "initial config should be set")

	// Overwrite with new config
	newConfig := &filesystemv1alpha1.Config{
		TempFolder: new("/tmp/overwrite"),
	}
	...
	ocmCtx = FromContext(ctx)
	fsCfg = ocmCtx.FilesystemConfig()
	r.Equal(new("/tmp/overwrite"), fsCfg.TempFolder, "config should be overwritten")
}
...
func TestFilesystemConfigIsolation(t *testing.T) {
	...
	config1 := &filesystemv1alpha1.Config{TempFolder: new("/tmp/ctx1")}
	config2 := &filesystemv1alpha1.Config{TempFolder: new("/tmp/ctx2")}
	...
	r.Equal(new("/tmp/ctx1"), fsCfg1.TempFolder, "context 1 should have correct config")
	r.Equal(new("/tmp/ctx2"), fsCfg2.TempFolder, "context 2 should have correct config")
