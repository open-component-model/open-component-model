// Step 1: Binary Data (Blobs)
//
// What you'll learn:
//   - Creating in-memory blobs from strings and byte slices
//   - Attaching metadata (size, digest, media type) to blobs
//   - Creating blobs from files on disk
//   - Copying blob data with automatic digest verification
//
// Blobs are the foundational data abstraction in OCM. Every resource stored in
// a component version — whether it's an OCI image, a Helm chart, or a plain
// file — is represented as a blob. Understanding blobs is the first step to
// working with OCM.
