// Package pypi contains helpers for PyPI Simple Repository API resolution and
// authenticated distribution transport over HTTP(S).
package pypi
...
// Client performs authenticated HTTP(S) transport of PyPI distribution files
// and the Simple Repository API pages that list them. It is a thin wrapper
// around *http.Client that turns typed PyPI credentials into an Authorization
...
// NewClient wraps httpClient for PyPI transport. A nil httpClient defaults to
...
// distribution can be streamed straight into an archive. accept, when
// non-empty, is sent as the Accept header (used for Simple API content
// negotiation).
func (c *Client) Get(ctx context.Context, url, accept string, credentials *credsv1.PyPICredentials) (*http.Response, error) {
...
func (c *Client) Put(ctx context.Context, url string, body io.Reader, size int64, contentType string, credentials *credsv1.PyPICredentials) error {
...
var ErrUnusableCredentials = errors.New("pypi credentials present but neither identityToken nor username is set")
...
// username/password, which yield Basic auth (an API token is passed as the
// "__token__" username with the token as the password).
func applyCredentials(req *http.Request, credentials *credsv1.PyPICredentials) error {
	if credentials == nil {
		return nil
	}
