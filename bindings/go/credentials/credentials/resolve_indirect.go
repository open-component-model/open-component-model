...
func (g *Graph) resolveIndirect(ctx context.Context, identity runtime.Identity) (map[string]string, error) {
	if credentials, cached := g.getFromCache(identity.String()); cached {
		return credentials, nil
	}

	plugin, err := g.getRepositoryPlugin(ctx, identity)
	if err != nil {
		return nil, fmt.Errorf("failed to find repository plugins for type %s: %w", identity.GetType(), err)
	}
	...
	g.addToCache(identity.String(), resolved)
