...
import (
	...
	"ocm.software/open-component-model/bindings/go/helm/transformer/spec/v1alpha1"
...
func (t *GetHelmChart) Transform(ctx context.Context, step runtime.Typed) (runtime.Typed, error) {
	...
	if helmAccess.HelmChart != "" {
		// if HelmChart is provided separately in the spec spec, append it to the repository URL to get the full URL for the chart.
	...
	chartFileSpec, err := filesystem.BlobToSpec(resultBlob.ChartBlob, chartOutputPath)
	if err != nil {
	...
	if resultBlob.ProvBlob != nil {
		provSpec, err := filesystem.BlobToSpec(resultBlob.ProvBlob, fmt.Sprintf("%s.prov", chartOutputPath))
		if err != nil {
