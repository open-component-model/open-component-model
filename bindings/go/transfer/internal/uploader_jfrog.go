...
// operands (see celOperand) templated like the HTTP uploader. A local blob source streams
// directly from the source component version (HTTPStreamingSpec.componentVersion).
func processJFrogHelmUploader(resource descriptorv2.Resource, access runtime.Typed, u *transferv1alpha1.JFrogHelmUploaderConfig, baseID, id string, val *discoveryValue, tgd *transformv1alpha1.TransformationGraphDefinition, resourceTransformIDs map[int]string, i int) error {
	...
	extra, err := jfrogHelmExtras(access, u, helmRepo, val)
	if err != nil {
		return err
	}
...
// the reindex request and, for a local blob, its source component version.
func jfrogHelmExtras(access runtime.Typed, u *transferv1alpha1.JFrogHelmUploaderConfig, helmRepo string, val *discoveryValue) (map[string]any, error) {
	...
	if u.ReindexEnabled() {
		...
		if err := wgetaccess.Scheme.Convert(&wgetaccessv1.Wget{Type: wgetaccess.V1VersionedType, URL: reindexURL, Verb: http.MethodPost}, reindex); err != nil {
			return nil, fmt.Errorf("cannot convert uploader reindex request: %w", err)
		}
		extra["afterUpload"] = reindex
	}
	if _, ok := access.(*descriptorv2.LocalBlob); ok {
		// The local blob streams straight from the source component version.
		sourceRepo, err := asUnstructured(val.SourceRepository)
		if err != nil {
			return nil, fmt.Errorf("cannot convert source repository spec to unstructured: %w", err)
		}
		extra["componentVersion"] = map[string]any{
