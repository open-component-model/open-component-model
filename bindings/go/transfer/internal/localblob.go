...
func processLocalBlob(resource descriptorv2.Resource, _ *descriptorv2.LocalBlob, id string, val *discoveryValue, tgd *transformv1alpha1.TransformationGraphDefinition, toSpec runtime.Typed, resourceTransformIDs map[int]string, i int, uploadAsOCIArtifact bool) error {
	...
	getResourceID, err := appendGetLocalResource(resource, id, val, tgd)
	if err != nil {
		return err
	}
...
// appendGetLocalResource appends the Get{OCI,CTF}LocalResource transformation that buffers the
// local blob of resource from the source component version to a file, and returns its ID.
func appendGetLocalResource(resource descriptorv2.Resource, id string, val *discoveryValue, tgd *transformv1alpha1.TransformationGraphDefinition) (string, error) {
	resourceIdentity := resource.ToIdentity()
	getResourceID := fmt.Sprintf("%sGet%s", id, identityToTransformationID(resourceIdentity))
	...
	getLocalResourceType, err := chooseGetLocalResourceType(val.SourceRepository)
	if err != nil {
		return "", fmt.Errorf("choosing get local resource type for source repository: %w", err)
	}
	sourceRepoUnstructured, err := asUnstructured(val.SourceRepository)
	if err != nil {
