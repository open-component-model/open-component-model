...
func TestNewFromBlob(t *testing.T) {
	t.Run("With Size and MediaType", func(t *testing.T) {
		testData := "test data from blob"
		originalBlob := NewFromBytes([]byte(testData), WithMediaType("text/plain"))

		// Create new blob from the original
		newBlob, err := NewFromBlob(originalBlob)
		require.NoError(t, err)

		// Verify size is preserved
		assert.Equal(t, int64(len(testData)), newBlob.Size())

		// Verify media type is preserved
		mediaType, known := newBlob.MediaType()
		assert.True(t, known)
		assert.Equal(t, "text/plain", mediaType)

		// Verify data is readable
		reader, err := newBlob.ReadCloser()
		require.NoError(t, err)
		t.Cleanup(func() {
	...
	t.Run("With ReadCloser Error", func(t *testing.T) {
		// Create a mock blob that returns error on ReadCloser
		mockBlob := &mockBlob{
			data:          "test data",
			readCloserErr: assert.AnError,
		}

		// Should return error when ReadCloser fails
		_, err := NewFromBlob(mockBlob)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "unable to open blob")
	})
}
