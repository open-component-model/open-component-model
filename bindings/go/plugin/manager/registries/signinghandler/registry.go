...
import (
	...
	"ocm.software/open-component-model/bindings/go/signing/handler"
)
...
func NewSigningRegistry(ctx context.Context) *SigningRegistry {
	return &SigningRegistry{
		...
		internalPlugins:    make(map[runtime.Type]handler.ComponentSignatureHandler),
...
type SigningRegistry struct {
	...
	internalPlugins    map[runtime.Type]handler.ComponentSignatureHandler
...
func (r *SigningRegistry) GetPlugin(ctx context.Context, spec runtime.Typed) (handler.ComponentSignatureHandler, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func RegisterInternalComponentSignatureHandler(
	scheme *runtime.Scheme,
	r *SigningRegistry,
	plugin handler.ComponentSignatureHandler,
	proto runtime.Typed,
) error {
	r.mu.Lock()
	defer r.mu.Unlock()
