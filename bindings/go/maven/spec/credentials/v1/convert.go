...
const (
	...
	// credentialKeyAccessToken is the token key old OCM read for Maven
	// repositories. Property bags written for it must keep working.
...
// ConvertToMavenCredentials converts runtime.Typed credentials into
// *MavenCredentials, accepting either a typed credential or a
// DirectCredentials/v1 property bag.
//
// Nil, an empty type, or a property bag without any Maven key yields nil
// without an error: most Maven repositories are readable anonymously, so
// absent credentials are not a failure. Rejecting credentials that are present
// but unusable is the caller's job.
func ConvertToMavenCredentials(creds runtime.Typed) (*MavenCredentials, error) {
	...
	case *MavenCredentials:
		return t, nil
	}

	return nil, fmt.Errorf("unsupported credential type for maven access: %v", typed.GetType())
}
...
// used only when the typed key is absent. A bag that holds none of the Maven
...
func fromDirectCredentials(properties map[string]string) *MavenCredentials {
