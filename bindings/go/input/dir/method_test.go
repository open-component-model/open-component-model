...
func Test_ensureAbsolutePath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dir := v1.Dir{Path: tt.args.path}
			err := ensureAbsolutePath(&dir, tt.args.workingDir)
			if !tt.wantErr(t, err, fmt.Sprintf("ensureAbsolutePath(%v, %v)", tt.args.path, tt.args.workingDir)) {
				return
			}

			assert.Equalf(t, tt.wantPath, dir.Path, "ensureAbsolutePath(%v, %v)", tt.args.path, tt.args.workingDir)
		})
	}
}
