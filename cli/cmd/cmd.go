...
func New() *cobra.Command {
  ...
  Component Repositories, and Component Versions.`,
	...
	cmd.PersistentFlags().Duration(ocmcmd.TCPDialTimeoutFlag, 0, `TCP dial timeout for establishing connections (e.g. "30s"). Use "0" to disable. Overrides config file value.`)
	cmd.PersistentFlags().Duration(ocmcmd.TCPKeepAliveFlag, 0, `TCP keep-alive interval (e.g. "30s"). Use "0" to disable. Overrides config file value.`)
	cmd.PersistentFlags().Duration(ocmcmd.TLSHandshakeTimeoutFlag, 0, `TLS handshake timeout (e.g. "10s"). Use "0" to disable. Overrides config file value.`)
	cmd.PersistentFlags().Duration(ocmcmd.ResponseHeaderTimeoutFlag, 0, `HTTP response header timeout (e.g. "10s"). Use "0" to disable. Overrides config file value.`)
	cmd.PersistentFlags().Duration(ocmcmd.IdleConnTimeoutFlag, 0, `HTTP idle connection timeout (e.g. "90s"). Use "0" to disable. Overrides config file value.`)
	cmd.PersistentFlags().Duration(ocmcmd.RetryMinWaitFlag, 0, `Minimum wait before retrying a failed HTTP request (e.g. "200ms"). Use "0" to disable. Overrides config file value.`)
	cmd.PersistentFlags().Duration(ocmcmd.RetryMaxWaitFlag, 0, `Maximum wait before retrying a failed HTTP request (e.g. "3s"). Use "0" to disable. Overrides config file value.`)
	cmd.PersistentFlags().Int(ocmcmd.RetryMaxRetryFlag, 0, `Maximum number of HTTP retry attempts. Use "0" to disable retries. Overrides config file value.`)
