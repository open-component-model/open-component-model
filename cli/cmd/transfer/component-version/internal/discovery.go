...
func (r *resolver) Resolve(ctx context.Context, key string) (*discoveryValue, error) {
	...
	if r.credResolver != nil {
		if consumerId, err := r.repoProvider.GetComponentVersionRepositoryCredentialConsumerIdentity(ctx, ref.Repository); err == nil {
			creds, err = r.credResolver.Resolve(ctx, consumerId)
			if err != nil {
	...
	repo, err := r.repoProvider.GetComponentVersionRepository(ctx, ref.Repository, creds)
	if err != nil {
