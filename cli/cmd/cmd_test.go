...
func Test_Sign_And_Verify_Component_Version_DefaultSpec(t *testing.T) {
	...
	name, version := "ocm.software/examples-default-spec", "1.0.0"
	constructorYAML := fmt.Sprintf(`
name: %[1]s
version: %[2]s
provider:
...
resources:
  - name: my-secure-resource
    ...
    input:
      type: utf8/v1
      text: "default spec signing test"
`, name, version)
	...
	// Credentials for the default RSASSA-PSS signer/verifier — no --signer-spec or
	// --verifier-spec flags are passed below, so the CLI must resolve the algorithm
	// from these credentials alone.
	ocmConfigYAML := fmt.Sprintf(`
type: generic.config.ocm.software/v1
configurations:
- type: credentials.config.ocm.software
...
`, signatureName, publicKeyChainPath, privateKeyPath)
	...
	r.NoError(err, "failed to sign with default spec")
