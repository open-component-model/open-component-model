...
func Test_Add_And_Transfer_Access_Type_Casing(t *testing.T) {
	tests := []struct {
		name           string
		accessType     string
		expectedAccess string
		...
		{
			name:           "helm/v1 access lowercase",
			accessType:     "helm/v1",
			expectedAccess: "helm/v1",
		},
		{
			name:           "Helm/v1 access upper camel case",
			accessType:     "Helm/v1",
			expectedAccess: "Helm/v1",
		},
	}
...
name: ocm.software/test-access-casing
version: 1.0.0
provider:
  name: ocm.software
resources:
...
`, tc.accessType)
			...
			// Step 1: add cv (access types don't resolve at add time)
			sourceArchive := filepath.Join(tmp, "source-archive")
			logs := test.NewJSONLogReader()
			_, err := test.OCM(t, test.WithArgs("add", "cv",
				"--constructor", constructorPath,
			...
			r.NoError(err, "add cv failed for access type %s", tc.accessType)
			...
			desc, err := sourceRepo.GetComponentVersion(t.Context(), "ocm.software/test-access-casing", "1.0.0")
			r.NoError(err, "could not get component version from source")
			r.Len(desc.Component.Resources, 1)
			r.Equal("my-chart", desc.Component.Resources[0].Name)
			r.Equal("helmChart", desc.Component.Resources[0].Type)
			r.Equal(tc.expectedAccess, desc.Component.Resources[0].Access.GetType().String(),
				"access type in descriptor should be normalized to default for %s", tc.accessType)

			// Step 2: transfer cv (CTF to CTF, no actual chart download needed for reference transfer)
			targetArchive := filepath.Join(tmp, "target-archive")
			sourceRef := fmt.Sprintf("ctf::%s//ocm.software/test-access-casing:1.0.0", sourceArchive)
			targetRef := fmt.Sprintf("ctf::%s", targetArchive)

			transferLogs := test.NewJSONLogReader()
			_, err = test.OCM(t, test.WithArgs("transfer", "component-version", sourceRef, targetRef),
				test.WithErrorOutput(transferLogs))
			r.NoError(err, "transfer cv failed for access type %s", tc.accessType)
			...
			targetDesc, err := targetRepo.GetComponentVersion(t.Context(), "ocm.software/test-access-casing", "1.0.0")
			r.NoError(err, "could not get component version from target")
			r.Len(targetDesc.Component.Resources, 1)
			r.Equal("my-chart", targetDesc.Component.Resources[0].Name)
			r.Equal("helmChart", targetDesc.Component.Resources[0].Type)
			r.Equal(tc.expectedAccess, targetDesc.Component.Resources[0].Access.GetType().String(),
				"access type in target should be normalized to default for %s", tc.accessType)
		})
	}
}
