...
func Register(manager *manager.PluginManager, logger *slog.Logger) error {
	...
	if err := ctfplugin.Register(manager.ComponentVersionRepositoryRegistry, logger); err != nil {
		return fmt.Errorf("could not register CTF inbuilt plugin: %w", err)
	}

	if err := file.Register(manager.InputRegistry, logger); err != nil {
		return fmt.Errorf("could not register file input plugin: %w", err)
	}
	if err := utf8.Register(manager.InputRegistry, logger); err != nil {
