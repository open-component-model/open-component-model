...
func TestTreeRendererStatic2(t *testing.T) {
	r := require.New(t)
	...
	// Mark B as discovered (still only root)
	vB.Attributes.Store(syncdag.AttributeTraversalState, syncdag.StateDiscovered)
	time.Sleep(30 * time.Millisecond)
	expected += "   ├─ B (discovered)\n"
	output = buf.String()
	r.Equal(expected, output)

	// Mark D as discovered (still only root)
	vB.Attributes.Store(syncdag.AttributeTraversalState, syncdag.StateDiscovered)
	time.Sleep(30 * time.Millisecond)
	output = buf.String()
	r.Equal(expected, output)

	// Mark C as discovered (still only root)
	vC.Attributes.Store(syncdag.AttributeTraversalState, syncdag.StateDiscovered)
	time.Sleep(30 * time.Millisecond)
	expected += "   │  ╰─ C (discovered)\n"
	output = buf.String()
	r.Equal(expected, output)
	...
	// Mark D as completed (still only root)
	vD.Attributes.Store(syncdag.AttributeTraversalState, syncdag.StateDiscovered)
	time.Sleep(30 * time.Millisecond)
	output = buf.String()
	expected += "   ╰─ D (discovered)\n"
	r.Equal(expected, output)
