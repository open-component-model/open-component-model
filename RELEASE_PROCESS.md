...
* At RC time the `tag_rc` step pushes `website/v0.X.Y-rc.N` and the `create_website_rc_update_pr` job dispatches the docs PR workflow with the RC version. The workflow registers the RC as a full-version entry (e.g. `0.16.0-rc.2`) on one stable PR branch per minor, so a later `rc.3` refreshes the still-open `rc.2` PR; `register-docs-version.js` drops the superseded RC's `hugo.yaml` entry and `module.yaml` imports, keeping only the latest RC live. RC entries are never the default version and are excluded from the 10-minor retirement; the final release PR removes the minor's remaining RC entry in the same commit.
