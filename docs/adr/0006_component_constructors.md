...
3. **Process Resources and Sources**  
   For each component version, the constructor handles all resources and sources:
    ...
    - **Access Specified**  
      If an access method is defined, it is applied directly to the component descriptor candidate.
...
// but if a ResourceMethodResult is returned, it MUST at least one of the above.
//
...
```go
// TargetRepository defines the interface for a target repository that can store component versions created 
// by the constructor but also act as a target for the constructor to add resources to.
