...
Technical Story: Define a single, operational specification for component versioning, release-branching, and CI publishing for the OCM monorepo.
...
Chosen option: Hybrid versioning + component-scoped persistent release branches (see justification below).
...
* Branching: Component-scoped persistent release branches — branches follow the pattern `releases/<component>/v<major>.<minor>`, e.g., `releases/cli/v0.30` and act as maintenance lines for each active minor release. Patches and hotfixes are applied via PRs to these branches.
...
We explicitly considered the following options before making the decision above. Recording them here improves traceability.
...
Rationale: Having both per-component `VERSION` files (working state) and a generated `/VERSIONS.yaml` provides submodules the autonomy to release independently and a single snapshot for consumers.
