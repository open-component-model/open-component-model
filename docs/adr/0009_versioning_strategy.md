...
The repository contains multiple independently developed components, e.g., `ocm-cli`, `ocm-controller`, and a root/wrapper component `ocm` (final name to be decided. Acts as a root component referencing versions of the sub-components).
...
* Support straightforward hotfix and backport workflows for older minor releases.
...
* Working state (pre-publication): `ocm-<component>/VERSION` located on the appropriate branch (either `main` for ongoing development, or a `releases/...` branch for maintenance lines).
...
* Read `ocm-<component>/VERSION` (fall back to latest component tag if absent)
