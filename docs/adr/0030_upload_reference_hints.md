# ADR: Portable Reference Hints and Typed Upload Configuration
...
```text
resource hint                 + uploader config       = target access
...
| Concern | Decision |
| --- | --- |
| Portable coordinates | List of typed, versioned hints in one resource label. No endpoints or credentials. |
...
| Hint cleanup | Remove after successful external upload **only if returned access preserves the required coordinates**. |
...
**A downloader needs a hint contract; it does not necessarily have an inverse uploader.**

| Access | Downloaded representation | Hint-list entry | Upload configuration |
| --- | --- | --- | --- |
| `OCIImage/v1` | OCI layout containing the selected artifact graph | `OCIReferenceHint/v1alpha1` | `oci.upload.config.ocm.software/v1alpha1` |
| `Helm/v1` | Chart + optional provenance, then converted to OCI layout | `OCIReferenceHint/v1alpha1`, after conversion | Same OCI config; no classic Helm config |
| `S3/v2` | Exact object bytes | `S3ReferenceHint/v1alpha1` | `s3.upload.config.ocm.software/v1alpha1`; requires a new upload backend |
| `Wget/v1` | HTTP response body | `WgetReferenceHint/v1alpha1` | None: download access does not specify an upload protocol |
| `GitHub/v1` | Exact commit tarball, `application/x-tgz` | `GitHubReferenceHint/v1alpha1` | None: a tarball cannot recreate a Git repository/commit |
| `LocalBlob/v1` | Existing stored representation | Preserve existing hint | Selected by hint; otherwise existing local storage behavior |
...
| `File/v1alpha1` | Transformation staging file | Preserve resource hint alongside file | None: internal blob locator |

Access aliases resolve through existing schemes. New hint/config types accept **only their canonical, versioned spelling**; no legacy aliases exist to support. Maven is absent from the current Go implementation and outside this API contract.

## Shared Label Envelope
...
Suggested generic representation in `bindings/go/descriptor/referencehint`:
...
```yaml
labels:
  - name: ocm.software/reference-hint
    signing: false
    value:
      - type: OCIReferenceHint/v1alpha1
        repository: acme/payments
        tag: "1.4.0"
      - type: MavenReferenceHint/v1alpha1
        groupId: com.acme
        artifactId: payments
        version: "1.4.0"
        extension: jar
...
* Exactly one reserved label. Its value is a JSON/YAML array of typed hint payloads. Each payload `type` owns versioning; omit `Label.Version`.
* At most one hint per target access family. The family key is `runtime.Type.Name`; versions of the same name therefore compete. Reject duplicate type names, regardless of version. Producers upsert their family entry rather than append blindly.
* List order has no precedence or selection meaning. Upload policy—including future resource matching—selects the uploader; that uploader selects its corresponding hint family.
* If multiple configured uploaders match a resource, fail unless configuration explicitly resolves the ambiguity. Never choose the first hint.
* Typed payloads are not partially valid access specs. No nested `access`, generic `properties`, or extra `format` field; each hint type fixes coordinate and representation semantics.
* Multiple coordinates do not imply conversion support. A hint is eligible only when the current blob representation is compatible with the selected uploader or a supported conversion produces that representation.
...
* Resource-aware transformers attach hints on copied output resources. `DownloadResource` remains a blob-returning API and must not mutate input metadata.
* Blob media type stays in `File.mediaType` / `LocalBlob.mediaType`; integrity stays in `resource.digest`. Do not duplicate these in every hint.
* Construct allowlisted relative fields rather than copying/redacting entire accesses. Hints are untrusted data, not CEL source or authority to choose an endpoint.

## OCI API

Suggested package: `bindings/go/oci/spec/referencehint/v1alpha1`.
...
Wire type: **`OCIReferenceHint/v1alpha1`**.
...
hint:   repository=team/app, tag=1.4.0, digest=sha256:<digest>
```

Parse a **full source reference** with the existing OCI parser; copy its repository/tag/digest. Do not parse a relative multi-segment repository as a full reference: that loses its first segment. A tagless/digestless source needs its selected root digest before a valid hint can be produced; never invent `latest`.
...
* Build `<baseUrl>/<hint.repository>[:<tag>][@<digest>]` structurally. Retain digest-only references; enforce the expected root digest before upload.
* Initially accept only the `OCIReferenceHint` entry. Preserve other entries; no implicit wrapping of arbitrary S3/Wget/GitHub blobs as OCI artifacts.
...
No separate persisted `HelmReferenceHint` for the **current transfer path**:

```text
GetHelmChart → ConvertHelmToOCI → OCIReferenceHint → AddLocalResource / AddOCIArtifact
```
...
Use resolved chart metadata, not requested version or OCM resource version. Reject invalid OCI repository names; do not silently lowercase/slugify. Preserve provenance inside the OCI layout. A future raw chart-bundle local-blob path needs a distinct hint type; never label a raw `.tgz` as an OCI layout.

## S3 API

Suggested package: `bindings/go/s3/spec/referencehint/v1alpha1`.
...
Target key = **literal `keyPrefix + hint.objectKey`**. Never filesystem-normalize it. Prefix containment is byte-prefix containment; S3 keys are not filesystem paths.
...
Initially consume only the `S3ReferenceHint` entry and preserve other entries. Ship this config with an S3 Add transformation and implementation of the currently unsupported `UploadResource`. If an intermediate build recognizes the config before execution support exists, reject it at config loading—including before controller allowlist filtering—rather than silently dropping it.

## Wget API

Suggested package: `bindings/go/wget/spec/referencehint/v1alpha1`.
...
```yaml
- type: WgetReferenceHint/v1alpha1
...
* Intentionally a **partial naming hint**: query-/body-/header-selected resources can share a path. It cannot authorize HTTP re-upload or uniquely identify downloaded bytes.
...
Suggested package: `bindings/go/github/spec/referencehint/v1alpha1`.
...
```yaml
- type: GitHubReferenceHint/v1alpha1
  owner: acme
  repository: payments
...
* Transfer already rejects ref-only accesses in `transfer/internal/github.go`. Preserve that requirement; do not resolve a branch again merely to construct the hint. Direct transformation calls must also avoid hints with unknown commits.
...
* `LocalBlob`: preserve the existing hint list, even with `globalAccess`; no `LocalBlobReferenceHint` or extra local-upload config. Existing transfer settings and component target control local storage.
* Legacy OCI layout: parse `referenceName` as a **relative** OCI repository/tag only when no OCI-family hint entry exists. A malformed or unsupported OCI-family entry is an error, not permission to fall back. Other-family entries do not disable the legacy OCI fallback. Never treat arbitrary tarballs as OCI layouts.
* `OCIImageLayer`: registered, but unsupported as a standalone resource by the current transfer planner/resource adapter. Commonly used as `LocalBlob.globalAccess`. Do not reinterpret it as `OCIImage` or replace an existing hint with container repository coordinates.
* `File`: carries staging URI/media type/digest between transformations. Never persist temporary paths as hints.
...
copyMode permits copying?
  → derive coordinates from access/conversion, or read local-blob hint
  → select compatible configured uploader and its hint-family entry
...
* Matching is configuration policy, not label semantics. Future matchers may select by resource identity or metadata. Select the uploader first, then look up its declared hint-family name; list order never breaks ties. Multiple matching uploaders without explicit precedence are an error.
...
* `BuildAndCheck` checks graph/schema/expression types; hint-specific and output-dependent validation must still run before upload.
* Dry-run may contain expressions; replay needs the graph and credentials/plugins, not original uploader config.

## Hint Lifecycle and Integrity

| Transition | Action |
| --- | --- |
| External → local | Derive or replace the current access family's hint; preserve other entries that remain valid for the produced representation. |
...
| Content conversion | Preserve, translate, or remove each entry according to compatibility with the resulting representation. Never carry known-invalid hints forward. |
| Failed upload | Leave source hint list and descriptor unchanged. |
...
* Cleanup runs **after backend success**, on a copied output resource. Remove only the consumed entry; remove the label only when its list becomes empty. Preserve unrelated labels, compatible hints, and shared source branches. Later descriptor-publication failure may leave uploaded artifacts; transfer is not transactional.
* Hints are transport metadata, not provenance. OCI/S3 prefixes become part of new accesses; subsequent downloads do not guess them away.
* Unsigned hint changes preserve descriptor normalization **if other signing-relevant fields remain unchanged**. Reject automatic mutation of signing-relevant hints.
...
| --- | --- |
| `descriptor/referencehint` (new) | `List` plus technology-neutral label decode/encode/find/upsert/remove helpers, copying, duplicate-family/signing guards. Family key is `runtime.Type.Name`; no technology imports. |
| `<technology>/spec/referencehint/v1alpha1` (new) | Four payloads above; registration, strict validation, generated schemas/deepcopy/type methods. |
...
| Technology transformations | Produce hints before access/resolved metadata is lost; copy output resources. |
...
**First increment:** OCI → hinted local blob → configured OCI upload; Helm-to-OCI hints; streaming parity. Add S3 upload support separately from S3 hint production.

**Constructor integration is separate:** S3/Wget inputs return only `ProcessedBlobData`; `constructor/construct.go` ignores `ProcessedResource` when blob data is also present. Metadata propagation must be explicit before claiming constructor-produced blobs automatically carry hints.
...
* Helm resolved metadata, repository paths, `+` → `_`, provenance, hints attached after conversion.
...
* Local hops, legacy fallback with non-OCI entries, unknown/invalid hints, conversion invalidation, per-entry cleanup, empty-label removal, failure/retry/multi-target isolation, signing invariance, unrelated metadata.
...
| --- | --- | --- |
| **Chosen: labels + typed configs** | Hint producers, config compiler, output cleanup. | Small UI; survives offline transfer. Adds a public metadata contract. |
...
| Graph-only hints | Pass coordinates between nodes without persistence. | No persistent metadata, but fails disconnected/multi-hop transfers. |
