...
**Technical Story**:
Define how the Open Component Model packages, transports, and delivers Virtual Machine (VM) disk images — `raw`, `qcow2`, `vmdk`, `vhd`, `vhdx`, `iso`, and OVA — together with their deployment configuration (cloud-init, ignition, OVF envelopes, sizing/placement metadata) as signed, self-describing, air-gap-capable component versions. This ADR settles the resource-type, media-type, access, and image↔config binding conventions for VM images whose blob fits within registry limits, and selects the large-blob splitting representation for 40 GiB+ images (a single OCI image manifest whose ordered layers are the chunks), whose implementation and remaining configuration questions are deferred to EPIC [`open-component-model/ocm-project#1290`](https://github.com/open-component-model/ocm-project/issues/1290).
...
* **Option 1 — Resource-typed convention with a media-type registry and label binding.** VM images are ordinary resources with dedicated PascalCase `type` strings and namespaced media types; embedded via the `File` input or referenced via `Wget` / `OCIImage` access; configuration is a sibling resource bound to the image via the spec's predefined `ocm.software/artifact-references` linking label. *Chosen.*
...
`blob` remains the generic fallback. This ADR defines two domain-specific artifact `type` strings, in PascalCase to match the access-type convention:
...
Resource `type` values are free-form strings in the component descriptor (like the existing `ociImage`, `helmChart`, `blob`, `git`); they are **not** Go constants, and none is required here. PascalCase is the current convention for typed identifiers — access types were standardized to it (`OCIImage`, `Wget`) — so new resource types follow the same style.
...
| Disk format              | OCM mediaType                                   | Known de-facto / recognized type                                                    |
|--------------------------|-------------------------------------------------|-------------------------------------------------------------------------------------|
| `raw`                    | `application/vnd.ocm.software.vm.disk.raw.v1`   | `application/x-raw-disk-image` (`freedesktop` shared MIME database)                 |
| `qcow2`                  | `application/vnd.ocm.software.vm.disk.qcow2.v1` | `application/x-qemu-disk` (`freedesktop`)                                            |
| `vmdk`                   | `application/vnd.ocm.software.vm.disk.vmdk.v1`  | `application/x-vmdk` (`freedesktop`)                                                 |
| `vhd`                    | `application/vnd.ocm.software.vm.disk.vhd.v1`   | `application/x-vhd` (`freedesktop`); Microsoft `vhd` (fixed/dynamic)                |
| `vhdx`                   | `application/vnd.ocm.software.vm.disk.vhdx.v1`  | Microsoft `vhdx` — no widely-registered media type                                 |
| `iso` (installer / live) | `application/vnd.ocm.software.vm.disk.iso.v1`   | `application/x-iso9660-image` (`freedesktop`)                                        |
| `ova` (tar of OVF+disks) | `application/vnd.ocm.software.vm.ova.v1+tar`    | `DMTF` OVF envelope (an `ova` is a tar of the OVF plus disks); no IANA subtype      |

`vhd` (fixed/dynamic VHD) and `vhdx` are distinct on-disk container formats that importers handle differently, so each gets its own media type and its own `software.ocm.vm/format` enum value (decision 5) rather than sharing one. The media-type suffix, the disk-format enum value, and the file extension therefore agree one-to-one.

No **IANA-registered** (standards-tree) media type exists for any VM disk format — the IANA `application` registry has no entry for `raw`, `qcow2`, `vmdk`, `vhd`, `vhdx`, ISO disk images, or OVA. Several formats do have **recognized de-facto or vendor types** (right-hand column above, mostly from the `freedesktop` shared MIME database plus `DMTF`'s OVF for OVA), but they are neither standardized, uniformly implemented, nor namespaced for OCM dispatch. This ADR therefore defines the reverse-DNS `application/vnd.ocm.software.vm.*` namespace as the **canonical dispatch key**, which keeps VM media types self-describing and consistent with the existing OCM layout media type `application/vnd.ocm.software.oci.layout.v1+tar` (`bindings/go/oci/spec/layout/media_type.go`), so `BlobTransformer` and download tooling ([ADR-0007](0007_resource_download.md)) can dispatch on them. Where interoperability with non-OCM consumers matters, tooling MAY additionally record the recognized de-facto type from the table above as an extra label — the OCM namespaced type stays authoritative for dispatch.
...
Tradeoff: not air-gap-portable; the URL must remain reachable at consume time. A `Wget` URL is **mutable** — the bytes it serves can change after packaging. The resource records a `digest` at construction (`ProcessResourceDigest` validates the response then), but that digest MUST also be re-verified on every lazy download: the consumption path passes the recorded digest to the downloader and rejects any mismatch before returning the blob, so a changed upstream response cannot reach the importer under the signed resource identity. Consumption-time digest verification is what makes an otherwise-mutable reference safe (decision 8).

#### Packaging Flow C — external reference via `OCIImage` access
...
A tag reference (`:24.04`) is pinned automatically: OCI digest processing at construction resolves the tag and rewrites the access to the resolved descriptor digest, so the recorded resource is immutable even though the input was tag-qualified. Publishers therefore need not pre-resolve a digest by hand — a tag in the constructor is sufficient, and the resulting descriptor carries the pinned digest.
...
**Association (image ↔ config binding).** The config resource is a *derived artifact* of the image resource, so it uses the spec's predefined [artifact-linking label](https://github.com/open-component-model/ocm-spec/blob/main/doc/01-model/06-conventions.md#artifact-linking-label) — `ocm.software/artifact-references` (label `version: v1alpha1`) — rather than a VM-specific link key. This ADR does **not** invent a resource→resource link; it reuses the one the spec already defines, along with its documented lookup algorithm and identity-selector matching. The label is placed on the derived (config) resource and its `value` is a list of `{ identity: { name, version, … } }` entries pointing back to the subject image resource; the image resource itself is unchanged.

The image resource additionally carries one **signing-relevant** VM-specific label declaring its disk format (`Label` struct in `bindings/go/constructor/spec/v1/constructor.go` — fields `name`, `value`, `signing`, `version`):

* `software.ocm.vm/format` on the image resource — value is the disk-format enum: `raw | qcow2 | vmdk | vhd | vhdx | iso | ova`. This is a self-describing format declaration, not a cross-artifact link, so it stays VM-specific.

Set `signing: true` on both the format label and the `ocm.software/artifact-references` binding label, so the declared format and the image↔config pairing are covered by the component signature and cannot be tampered with independently of the signed component version.

### 6. Transport of large images

VM images are routinely 2–40 GiB. Transport MUST use the streaming path from [ADR-0022](0022_streaming_resource_content.md) (`bindings/go/oci/stream`, `ResourceStream`, `oras.CopyGraph`), which moves content blob-by-blob with existence checks (layer deduplication) and never fully materializes the image in memory or `/tmp`. The OCI-registry backend benefits today. The CTF backend stores blobs as individual files, but true streaming for CTF is **not yet implemented** — tracked separately from this ADR; until it lands, large-image transport to/from a CTF may still buffer. This ADR mandates the streaming contract; closing the CTF gap is follow-up work.
...
* The deploying platform (a hypervisor driver, Kubernetes CDI/KubeVirt, or a cloud image importer) reads the `software.ocm.vm/format` label to select the import routine and resolves the paired `VirtualMachineConfig` resource by running the artifact-reference lookup algorithm — finding the config resource whose `ocm.software/artifact-references` label points at the image's identity.
...
graph TD
    ...
    CFG["Resource: config<br>type=VirtualMachineConfig<br>label ocm.software/artifact-references→disk"]:::ocm

    CV --> IMG
    CV --> CFG

    IMG -->|"File input (embedded) OR<br>Wget/OCIImage (referenced)"| DESC["Signed Descriptor"]:::ocm
    ...
    DL -->|"artifact-references resolves config"| PLAT
```

### 9. Large-Blob Splitting (representation chosen — implementation deferred to EPIC #1290)

> This subsection records the problem and the candidate solutions for images that exceed a registry's push/layer size limit, and **selects the representation**: a single OCI image manifest whose ordered layers are the chunks. The split trigger, the chunk-boundary strategy, the chunk metadata, and the implementation are left to EPIC [#1290](https://github.com/open-component-model/ocm-project/issues/1290).
...
1. **OCI image manifest with chunk layers.** `localBlob.localReference` points at a single OCI image manifest (`application/vnd.oci.image.manifest.v1+json`) whose ordered `layers` are the chunks. Reassembly concatenates the layers in manifest order.
   * *Pro:* maximally OCI-native and the simplest structure — no extra indirection blob, no index. Our descriptors already look like image manifests, so consumers need no new top-level object type. The OCI descriptor per layer already carries `digest` and `size`; ordering is the layer order; extra per-chunk metadata (e.g. `chunkIndex`) rides in OCI annotations. Reuses [ADR-0013](0013_oci_format_compatibility.md) machinery.
   * *Con:* per-chunk ordering/metadata lives in the manifest layer list rather than a dedicated descriptor.
2. **Split-manifest indirection.** `localBlob.localReference` points to a small split-descriptor blob with a dedicated media type (proposed `application/vnd.ocm.software.blob.split.v1+json`) that lists the ordered child-layer digests, per-chunk sizes, and the total reassembled digest. Reassembly reads the descriptor, then streams the chunk layers.
   * *Pro:* keeps `LocalReference` single-valued (matches today's `v2.LocalBlob.LocalReference string` in `bindings/go/descriptor/v2/local_access.go`); whole-blob digest lives in the descriptor. This is #12's "blob descriptor with a dedicated media type" variant.
   * *Con:* one extra fetch to read the split descriptor; introduces a new media type; duplicates information an OCI manifest's layer list already carries.
3. **Digest list in `localReference`.** Overload `LocalReference` to carry an ordered list of chunk-layer digests directly (the variant mandelsoft favored in #12).
...
4. **OCI Image Index of chunks.** `localReference` points at an OCI Image Index (`application/vnd.oci.image.index.v1+json`) whose child manifests are the chunks.
   * *Pro:* OCI-native.
   * *Con:* an index is unnecessary here — a plain manifest with layers already represents the whole blob. It also overloads "index" for pure byte-chunking and risks confusing OCI-native consumers that interpret an index as a multi-arch image.

**Decision — representation 1, a single OCI image manifest with chunk layers.** The #1290 discussion asked for a *technology-specific* solution rather than a general one, on the grounds that blob storages have no problem with large files and only registries impose the limit. Representation 1 is exactly that: the splitting lives entirely in the OCI layer, reuses the ADR-0013 machinery, and leaves every non-OCI backend untouched. It is also the simplest OCI-native form — the whole blob is already representable by a manifest with layers, so no OCI Image Index (representation 4) and no OCM-level indirection blob or overloaded `localReference` (representations 2 and 3) are needed. Representations 2 and 3 invent an OCM-level construct for what is a registry-specific constraint; representation 4 adds an index tier that buys nothing over a plain manifest and risks being misread as a multi-arch image.

The manifest carries an OCM-specific artifact type / annotation so a consumer can distinguish a chunked-blob manifest from an ordinary image manifest, and the CTF path stores the chunk layers as individual files exactly as it already does for any other blob.
...
**Open — chunk-boundary strategy.** `maxLayerSize` is an **upper bound** on chunk size, not the chunk size itself. Two boundary strategies are on the table, to be chosen in the EPIC:

* **Fixed-size boundaries** — cut every `maxLayerSize` bytes. Simplest; a small edit near the front of the image shifts all subsequent boundaries and forces a near-complete re-upload.
* **Content-defined chunking** (rolling-hash boundaries, borg/restic-style), with `maxLayerSize` as the ceiling. A minimal change to a large image touches only the affected chunks, so unchanged chunks dedupe on re-upload ([@jneisener](https://github.com/open-component-model/open-component-model/pull/3595#issuecomment-5619906576)). More complex.

Regardless of strategy, every chunk MUST be **≤ `maxLayerSize`**, and chunk order MUST be recorded so reassembly is deterministic — for fixed-size boundaries the order is the byte offset; for content-defined boundaries the producer records an explicit `chunkIndex` per layer.

**Open — per-chunk metadata.** Each layer descriptor already carries `digest` and `size`. #1290 suggests recording `chunkIndex` (reassembly order) as an OCI annotation. The exact annotation set is settled in the EPIC.
...
* Every chunk is **≤ `maxLayerSize`** and its position in the whole blob is unambiguous, so reassembly is deterministic irrespective of the boundary strategy.
* **CTF and OCI backends** both supported (CTF stores chunks as individual files via `oras.CopyGraph`, per ADR-0022 — subject to the CTF streaming gap noted in decision 6).
...
* The image↔config binding is a label convention (`ocm.software/artifact-references`), not a schema-enforced relation. This is the spec's own predefined linking label with a documented lookup algorithm, not a construct invented here, and it is `signing: true` so tampering is detectable; but it is still a soft link a consumer must resolve rather than a hard descriptor edge.
...
OCM ships VM images and their configuration as ordinary signed resources: dedicated `VirtualMachineImage` / `VirtualMachineConfig` types, a namespaced `application/vnd.ocm.software.vm.*` media-type registry, embedded (`File`) or referenced (`Wget` / `OCIImage`) access, streaming transport for large blobs, a `software.ocm.vm/format` label declaring the disk format, and the spec's `ocm.software/artifact-references` linking label binding configuration to its image. This delivers verified, self-describing, air-gap-capable VM artifacts with no change to the component-descriptor schema. Images that exceed registry size limits are split transparently into a single OCI image manifest whose ordered layers are the chunks; the implementation and its remaining configuration questions are deferred to EPIC #1290.
