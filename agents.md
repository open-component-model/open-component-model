...
- Changes must align with the architecture described in `doc.go` files of each module and package if they exist.
...
Study the `bindings/go/runtime/` package and its `doc.go` for a full understanding.
...
// GetType() and SetType() are auto-generated by ocmtypegen into zz_generated.ocm_type.go.
```

Key concepts:

- **`runtime.Scheme`** — a thread-safe registry mapping `runtime.Type` → `reflect.Type`. Types are registered with `MustRegisterWithAlias` (explicit name) or `MustRegister` (derived from struct name).
- **`runtime.Typed` interface** — requires `GetType()`, `SetType()`, and `DeepCopyTyped()`. The first two are auto-generated by `ocmtypegen`.
- **Aliases** — a type can be registered with a default (versioned) name and an alias (unversioned). Resolution tries the default first, then aliases.
- **Instance creation** — `scheme.NewObject(type)` looks up the type, clones a prototype via `DeepCopyTyped()`, and calls `SetType()` on the result.

### CI Pipeline

- CI discovers Go modules dynamically and filters based on changed files
