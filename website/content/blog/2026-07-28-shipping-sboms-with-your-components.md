...
**Linking an SBOM to what it describes.** A resource of `type: sbom` carries an `ocm.software/artefactReference` label whose `identitySelector` points at the resource the SBOM is about. Because it is an ordinary, signed resource, the link travels and is verified with the component — no side channel.
...
One command produces a bill of materials for the whole component; the next answers "am I affected?" — from the delivered component, online or air-gapped, with no per-artifact hunting. We also explored teaching [OpenDeliveryGear (ODG)](https://open-component-model.github.io/open-delivery-gear/) to consume the SBOMs (with VEX) directly, so that vendor-provided triage data is applied automatically.
