...
this writing, it has 11 components that must be bundled together so the entire stack can be deployed is one.
...
component version references like this (warning, rather long yaml file incoming):
...
Once the component is created properly we transferred it to a new location:
...
The `--upload-as ociArtifact` here was important, because all the images and resources where OCI artifacts and [oras](https://oras.land/)
...
... we were able to deploy the entire application architecture without problems with all the image referenced
...
The component constructor YAML file contains a Schema file located here: [component constructor schema](https://ocm.software/docs/reference/component-constructor/).
This can be provided to the LLM as a basis of generating a valid component version. It doesn't really require any
...
Claude already has in-built JSON schema skill.
...
Before giving it the samples however, you can also fine-tune it's understanding about Kro. Link the RGD schema definition
...
than point it to a website like the rendered schema on Kro's page here: [Rendered RGD](https://kro.run/api/crds/resourcegraphdefinition/). This is
...
Once it understands it, we can move on to the next step.

#### The Next Step

With this done, now you can begin ocmifying your application. Give it the following prompt:

> Analyze the deployment and packaging process of this application. (You can be more specific here if you have a helm
> chart just point it at it.). Now, take this information and create a component constructor file that packages all of
> the application components as component references.

Notice that we aren't doing everything in one step. That just overloads the context. So once the component constructor
exists and it looks good, now you can move on to the deployment part.

> Create a bootstrapping structure for the application component version. The RGD should localize any and all image references
...
component constructors for each individual components. The idea here was that separate components could be exchanged for
organization specific ones. For example, if you had your own gpu-operator implementation that used a specific, platform
dependent implementation that you should be able to configure the deployment to use that instead.

Basically creating a plug-and-play style architecture for the components where a component can be exchanged for a different
...
And further, if you do change a component and an inevitable failures occurs, Support wouldn't be able to help you because
...
_every_ separate component. The bootstrap suddenly was 11 x 3 (eleven times the component, resource, deployer) to deploy
