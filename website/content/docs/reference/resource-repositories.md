...
This page is the technical reference for built-in resource repositories. For an introduction to what resource repositories are and why they exist, see [Concept: Resource Repositories]({{< relref "docs/concepts/resource-repositories.md" >}}).
...
| Access Type | Aliases |
|---|---|
| [`OCIImage/v1`]({{< relref "input-and-access-types.md" >}}#ocimagev1) | `ociArtifact`, `ociRegistry`, `ociImage` |
...
This identity is then matched against configured consumers in the credential system. See [Credential Consumer Identities: OCIRegistry]({{< relref "credential-consumer-identities.md" >}}#ociregistry) for matching rules.
...
The OCI resource repository also implements digest processing. After a resource is stored, OCM can query the registry to retrieve and verify the artifact's digest, ensuring the access specification is pinned to an immutable reference.
...
{{< callout type="note" >}}
For Helm charts stored in OCI registries, use the [OCI resource repository](#oci-resource-repository) with an [`OCIImage/v1`]({{< relref "input-and-access-types.md" >}}#ocimagev1) access type instead.
{{< /callout >}}
...
See [Credential Consumer Identities: HelmChartRepository]({{< relref "credential-consumer-identities.md" >}}#helmchartrepository) for matching rules.
...
See [Concept: Plugin System]({{< relref "docs/concepts/plugin-system.md" >}}) for details on building and installing plugins.
