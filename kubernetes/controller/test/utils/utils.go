...
// Run executes the provided command within this context, streaming output to GinkgoWriter using io.Pipe, and properly tracks goroutines.
func Run(cmd *exec.Cmd) ([]byte, error) {
	...
	GinkgoLogr.Info(fmt.Sprintf("running command: %s", command))
	...
	// Buffer to capture all output and wait group for goroutines
	...
	go func() {
		defer wg.Done()
		io.Copy(io.MultiWriter(&buf, GinkgoWriter), stdoutReader)
	}()

	// Stream stderr
	go func() {
		defer wg.Done()
		io.Copy(io.MultiWriter(&buf, GinkgoWriter), stderrReader)
	}()

	// Start the command
	if err := cmd.Start(); err != nil {
		// Close writers to avoid leaks
		if err := errors.Join(stdoutWriter.Close(), stderrWriter.Close()); err != nil {
			GinkgoLogr.Error(err, "error closing command output writers")
		}
		return nil, fmt.Errorf("%s failed to start: %w", command, err)
	}

	// Wait for the command to finish
	err := cmd.Wait()

	// Close writers to signal readers and finish copying
	if err := errors.Join(stdoutWriter.Close(), stderrWriter.Close()); err != nil {
		GinkgoLogr.Error(err, "error closing command output writers")
	}
