...
func TestResolveComponentVersion_ValidationErrors(t *testing.T) {
	...
	resolver := resolution.NewResolver(k8sClient, logger, nil, resolution.DefaultResolverOptions())
...
func setupTestEnvironment(t *testing.T, k8sClient client.Reader, logger logr.Logger) *testEnvironment {
	...
	resolver := resolution.NewResolver(k8sClient, logger, pm, resolution.DefaultResolverOptions())
